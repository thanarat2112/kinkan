...
<html lang="en">
    ...
    <br><br><br><br>
    <center><h1>ชำระแล้ว</h1></center>
  <center><a href="reportall.php"><button class="btn btn-outline-primary">สถานะการสั่งซื้อทั้งหมด</button></a> <button class="btn btn-outline-success">ชำระแล้ว</button> <a href="recan.php"><button class="btn btn-outline-danger">ยกเลิกแล้ว</button></a> <a href="report.php"><button class="btn btn-outline-warning">ยังไม่ได้ชำระ</button></a></center>
  <form class="d-flex" method="POST" action="reportall.php">
    ...
    <table class="table table-hover">
        ...
        <?php
         ...
         if($key_word !=""){
            $read = "SELECT * FROM tb_order where order_status='2'='$key_word' OR cus_name LIKE '%$key_word%' ORDER BY order_id DESC";
         }else{
            $read = "SELECT * FROM tb_order where order_status='2' order by reg_date DESC";
        }
        ...
        <td><td align="center"><a href="reportdtcon.php?order_id=<?php echo $data['order_id'];?>"><button class="btn btn-outline-success">รายละเอียด</button></a></td></td>
        </tr>
        <?php } ?>
    </table></cente>
    </div>
